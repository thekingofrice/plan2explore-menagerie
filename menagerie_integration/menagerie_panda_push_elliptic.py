"""EXPERIMENT: Menagerie Panda Push with an elliptic friction cone.

Throwaway copy of menagerie_panda_push.py, identical except for model.opt.cone. Exists to test
whether the elliptic cone avoids the SIGFPE that kills mj_step under the default pyramidal cone.
Delete once the question is settled -- the real change belongs in render_scene.load_model so
every task and arm inherits it, not in a forked wrapper.
...
class MenageriePandaPushElliptic(gym.Env):
    ...
    def __init__(
        ...
        # THE ONE CHANGE. mjOption is read at every step, so setting it post-compile needs no
        # recompile. Pyramidal (MuJoCo's default) approximates the friction cone with a
        # polyhedron and builds 2*(condim-1) paired constraint rows per contact; elliptic
        # represents the true cone with condim rows and never runs that pairing code -- which
        # is the only path that changed between the sweep runs that faulted and the one that
        # did not.
